...
app.post('/', async (req, res) => {
    ...
    const grades = [
        parseInt(req.body.math.trim(), 10) || 0,
        parseInt(req.body.english.trim(), 10) || 0,
        parseInt(req.body.hebrew.trim(), 10) || 0,
        parseInt(req.body.bible.trim(), 10) || 0,
        parseInt(req.body.culture.trim(), 10) || 0,
        parseInt(req.body.physics.trim(), 10) || 0,
        parseInt(req.body.computerscience.trim(), 10) || 0,
        parseInt(req.body.history.trim(), 10) || 0,
        parseInt(req.body.citizenship.trim(), 10) || 0,
    ];
    ...
    if (!grades.every(grade => !isNaN(grade))) {
        // Redirect to the same page with error message as query parameter
        return res.redirect('/?error=Please enter valid numeric grades less than or equal to 100');
    }
    ...
    try {
        ...
        redisClient.quit();
        res.sendFile(path.join(__dirname, 'public', 'index.html'), { name, gpa, redisData: redisKey });
        // res.render('index', { name, gpa, redisData: redisKey });
        console.log('Data has been stored in REDIS!');
    } catch (error) {
        console.error('Error storing data in Redis:', error);
        res.status(500).send('Internal Server Error');
    }
    ...
    redisClient.keys('*', (err, keys) => {
        ...
        keys.forEach(key => {
            redisClient.get(key, (err, value) => {
                ...
                if (counter === keys.length) {
                    redisClient.quit(() => {
                        // Render the HTML page using EJS template
                        res.render('keys', { data });
                    });
                }
            });
        });
